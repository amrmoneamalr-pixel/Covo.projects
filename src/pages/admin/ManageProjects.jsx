...
import { Loader2, Save, Upload, Wand2, Trash2, ChevronDown, X, Image as ImageIcon, FileText } from 'lucide-react'
...
import BrochureExtractor from '../../components/BrochureExtractor'
...
export default function ManageProjects() {
  ...
  const refreshSelected = async () => {
    if (!selected) return
    const { data: p } = await supabase.from('projects').select('*').eq('id', selected.id).maybeSingle()
    if (p) { setSelected(p); setForm(p) }
    const { data: ls } = await supabase.from('unit_layouts').select('*').eq('project_id', selected.id)
    setLayouts(ls || [])
  }
  ...
  const save = async () => {
    ...
    const patch = {
      name: form.name, phase: form.phase,
  ...
  const addLayout = async (file, meta) => {
    ...
    try {
      ...
      const row = {
        ...
        floor_type: meta.floor_type, image_url: url,
      }
  ...
  const runMatch = async () => {
    ...
    const { data: units } = await supabase.from('units').select('*').eq('project_id', selected.id)
  ...
  const saveDeveloperLogo = async (file) => {
    ...
    setDevSaving(true); setDevEditMsg('Uploading logo…')
    try {
      const path = `developers/${editDev.id}/logo-${Date.now()}-${file.name}`
      const url = await uploadAsset(file, path)
      const { error: devErr } = await supabase.from('developers').update({ logo_url: url }).eq('id', editDev.id)
      if (devErr) throw devErr
      const { error: projErr } = await supabase.from('projects').update({ logo_url: url }).eq('developer_id', editDev.id)
    ...
    } catch (e) { setDevEditMsg('Error: ' + e.message) }
    finally { setDevSaving(false) }
  }

  const saveDeveloperName = async (newName) => {
    if (!editDev || !newName.trim()) return
    setDevSaving(true); setDevEditMsg('Renaming…')
    try {
      const trimmed = newName.trim()
      const { error: devErr } = await supabase.from('developers').update({ name: trimmed }).eq('id', editDev.id)
      if (devErr) throw devErr
      const { error: projErr } = await supabase.from('projects').update({ developer_name: trimmed }).eq('developer_id', editDev.id)
      if (projErr) throw projErr
  ...
  const filteredDevsForDropdown = useMemo(() => {
    ...
    return developers.filter((d) => d.name.toLowerCase().includes(devSearch.toLowerCase()))
  }, [developers, devSearch])

  const activeDev = useMemo(() => developers.find((d) => d.id === devFilterId) || null, [developers, devFilterId])
          ...
          <div className="max-w-2xl">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-2">
                {/* NEW: Smart Brochure button */}
                <button
                  onClick={() => setShowBrochure(true)}
                  className="flex items-center gap-2 bg-covo-teal text-black text-sm font-semibold px-4 py-2 rounded-lg hover:opacity-90"
                >
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <select value={form.type || 'residential'} onChange={(e) => set('type', e.target.value)}
      ...
      <div className="bg-bg-card border border-line rounded-lg shadow-2xl w-full max-w-md">
        ...
        <div className="px-5 py-4 space-y-4">
          <div className="flex items-center gap-3 p-3 bg-bg-base border border-line rounded-md">
            <div className="w-14 h-14 rounded-full bg-white border border-line flex items-center justify-center overflow-hidden shrink-0">
              {dev.logo_url ? (
                ...
                <span className="text-covo-gold font-bold text-lg">{(dev.name || '?').charAt(0).toUpperCase()}</span>
              )}
            </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Developer name"
              ...
              <button onClick={() => onRename(name)} disabled={saving || !name.trim() || name.trim() === dev.name}
                className="bg-covo-gold hover:opacity-90 disabled:opacity-30 text-black text-xs font-semibold px-3 rounded-md">Rename</button>
            </div>
          </div>
          <div>
            <label className="block text-[11px] font-semibold text-ink-muted mb-1.5">Upload New Logo (image)</label>
            <div className="flex gap-2">
              ...
              <button onClick={() => { onUploadLogo(file); setFile(null) }} disabled={saving || !file}
          ...
          {msg && (
            <p className={`text-xs ${msg.startsWith('Error') ? 'text-covo-pink' : 'text-covo-teal'}`}>{msg}</p>
          )}
        </div>
        <div className="px-5 py-3 border-t border-line flex justify-end">
          <button onClick={onClose} className="bg-bg-base border border-line text-ink-muted text-xs font-semibold px-4 py-2 rounded-md hover:text-ink">Close</button>
        </div>
      </div>
    </div>
  )
...
function AddLayoutForm({ onAdd }) {
  ...
  return (
    <div className="bg-bg-card border border-line rounded-lg p-3 grid grid-cols-3 gap-2 items-end">
      ...
      <button onClick={() => { onAdd(file, meta); setMeta({ name: '', category: '', bedrooms: '', bua: '', floor_type: '' }); setFile(null) }}
